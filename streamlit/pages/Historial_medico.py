...
def app():
    ...
    if not pacientes_db:
        st.warning("⚠️ No hay pacientes registrados. Antes de nada, vaya a 'Registrar Cliente' para empezar.")
        st.stop()
    ...
    #TAB 1: VER HISTORIAL
    with tab_ver:
        # Leemos todo el historial del paciente ordenado por fecha 
        query_historial = """
    ...
    with tab_anadir:
        ...
        with st.form("form_historial"):
            ...
            if submitted:
                # Dejamos ver el tipo con la descripción para que se vea claro en la tabla
